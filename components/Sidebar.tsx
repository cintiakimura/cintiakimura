...
import { Upload, X, FileText, File as FileIcon, Trash2, Plus, XCircle, MessageSquare, Sparkles } from 'lucide-react';
...
interface SidebarProps {
  ...
  onChangeView: (view: 'chat' | 'studio') => void;
}
  ...
  return (
    <div className="flex flex-col h-full bg-notebook-surface">
      <div className="p-4 border-b border-notebook-border flex items-center justify-between">
        <h2 className="font-semibold text-slate-200">Notebook</h2>
      ...
      <div className="p-4 grid grid-cols-2 gap-2">
        <button 
          ...
          className={`flex items-center justify-center gap-2 p-2 rounded-lg text-sm font-medium transition-colors ${
            currentView === 'chat' 
              ? 'bg-notebook-item text-white border border-notebook-border' 
              : 'text-slate-400 hover:text-slate-200 hover:bg-notebook-item/50'
          }`}
        >
        ...
        <button 
          ...
          className={`flex items-center justify-center gap-2 p-2 rounded-lg text-sm font-medium transition-colors ${
            currentView === 'studio' 
              ? 'bg-gradient-to-r from-purple-500/20 to-pink-500/20 text-white border border-notebook-border border-purple-500/30' 
              : 'text-slate-400 hover:text-slate-200 hover:bg-notebook-item/50'
          }`}
        >
          <Sparkles size={16} className={currentView === 'studio' ? 'text-purple-400' : ''} />
          Studio
        </button>
      </div>

      <div className="px-4 pb-2">
        <div className="h-px bg-notebook-border"></div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-2">
        {files.length === 0 ? (
          <div className="text-center py-4 px-2 text-slate-500 text-sm">
